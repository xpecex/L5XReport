...
- **`src/main/bypass.js`** — Worker script (runs in a separate `worker_threads` thread). Receives `filepath` and `CONFIG` via `workerData`. Parses L5X XML with Cheerio (`xmlMode`), iterates Routines→Rungs, matches bypass patterns, posts `PROGRESS`/`SUCCESS`/`ERROR` messages to the main thread. Skips 'Empty' routines.
