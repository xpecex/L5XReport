 ...
 * @param {string} locale - Browser locale string (e.g. 'pt-BR').
...
/**
 * Format a date string to locale-aware date-time.
 * @function formatAuditDateTime
 * @param {string} dateStr - ISO date string from the audit timestamp.
 * @param {string} locale - Browser locale string (e.g. 'pt-BR').
 * @returns {string} Formatted date-time or original string if invalid.
 */
function formatAuditDateTime(dateStr, locale) {
    if (!dateStr) return '';
    const d = new Date(dateStr);
    if (isNaN(d.getTime())) return dateStr;
...
function renderLevelBadge(level) {
    ...
    const label = safety ? i18n.t('report.level_safety') : i18n.t('report.level_standard');
...
function render() {
    ...
    // Totals
    const totalProgramsEl = document.querySelector('span#totalPrograms');
    if (totalProgramsEl) totalProgramsEl.textContent = totalPrograms;
    const totalRoutinesEl = document.querySelector('span#totalRoutines');
    ...
    const backupDate = results.length > 0 ? formatDateTime(results[0]['BACKUP'], browserLocale) : '';
    const auditDate = results.length > 0 ? formatAuditDateTime(results[0]['AUDIT. DATA'], browserLocale) : '';
    const infoBar = document.querySelector('.flex.justify-between.bg-slate-50');
    if (infoBar) {
        infoBar.innerHTML = `
            <div><span class="font-bold text-slate-500">${i18n.t('report.backup_date')}</span> ${backupDate}</div>
            <div><span class="font-bold text-slate-500">${i18n.t('report.audit_date')}</span> ${auditDate}</div>
        `;
    }
    ...
    if (pieChart) {
        ...
        if (legend) {
            legend.innerHTML = `
                <div class="flex items-center"><span class="w-3 h-3 bg-[#B91C1C] mr-1 rounded-sm"></span> ${i18n.t('report.level_safety_label')} (${pie.pctB}%)</div>
                <div class="flex items-center"><span class="w-3 h-3 bg-[#2E5A97] mr-1 rounded-sm"></span> ${i18n.t('report.level_standard_label')} (${pie.pctA}%)</div>
            `;
        }
    }
    ...
    if (footer) {
        ...
        footer.innerHTML = `
            <span>${i18n.t('report.footer_generated')} <a href="https://github.com/xpecex/L5XReport" target="_blank">L5XReport</a></span>
...
$('#btn-print').addEventListener('click', async () => {
    const btn = $('#btn-print');
    btn.disabled = true;
    btn.textContent = i18n.t('report.btn_print_generating');

    const result = await ipc.printPdf();

    if (result.success) {
        btn.textContent = i18n.t('report.btn_print_saved');
        setTimeout(() => { btn.textContent = i18n.t('report.btn_print'); btn.disabled = false; }, 2000);
    } else {
        btn.textContent = i18n.t('report.btn_print');
        btn.disabled = false;
        if (result.error) {
